...
protocol EditorialTableCellErrorViewController : class {
  var error: Error { set get }
}

enum EditorialTableViewCellState {
  case initial
  case loading
  case failure(error: Error)
  case success(models: [EditorialTableViewCellModel])
}

class EditorialTableViewCellStateController: UIViewController {
  typealias ErrorViewControllerType = EditorialTableCellErrorViewController & UIViewController
  ...
  private let successController: EditorialTableViewController
  ...
  public init(initialViewController: UIViewController,
       ...
       successController: EditorialTableViewController) {
  ...
  private func render(_ state: EditorialTableViewCellState) {
    children.forEach {
      $0.removeFromParent()
    }
